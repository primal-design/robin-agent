...
export const TRIGGERS = [
  {
    ...
    message: (ctx) =>
      `Hey — you've been quiet for a bit. Last thing you did was ${ctx.facts.slice(-1)[0] || 'get started'}. Still on it? 🦊`
  },
  {
    name: 'streak_at_risk',
    check: (ctx) => ctx.streak_at_risk,
    message: (ctx) =>
      `Your ${ctx.streak}-day streak ends in ${Math.round(24 - ctx.silence_hours)} hours — what's one thing you can do right now? 🦊`
  },
  {
    name: 'stuck_at_zero',
    check: (ctx) => ctx.stuck && !ctx.goal,
    message: () =>
      `You haven't started yet — that's fine. Tell me one thing you're good at. Anything. 🦊`
  },
  {
    name: 'near_first_win',
    check: (ctx) => ctx.near_first_win && ctx.silence_hours > 12,
    message: (ctx) =>
      `You're ${ctx.tasks_done} tasks in — you're close to that first £100. What happened today? 🦊`
  },
  {
    name: 'monday_reset',
    check: (ctx) => ctx.is_monday && ctx.is_morning && ctx.silence_hours > 8,
    message: (ctx) =>
      `New week — what's the ONE thing that would make this week a win for your ${ctx.niche || 'hustle'}? 🦊`
  }
]
...
// ── Approval execution ────────────────────────────────────────────────────
// When user says yes/go/do it — Robin executes immediately
export async function handleApproval(sessionId, pendingAction, session) {
  ...
  switch (pendingAction.type) {
    ...
    case 'send_outreach': {
      // For now: return the drafted message ready to copy-paste
      // Future: integrate with email/WhatsApp API
      results.push({
        executed: true,
        channel: 'draft_ready',
    ...
    case 'create_stripe_link': {
      // Returns instructions since Stripe API needs keys
      results.push({
        executed: false,
...
export async function craftMessage(ctx, instruction) {
  const response = await ai.messages.create({
    ...
    messages: [{
      role: 'user',
      content: `User context: ${JSON.stringify(ctx)}\n\nInstruction: ${instruction}`
    }]
  })
