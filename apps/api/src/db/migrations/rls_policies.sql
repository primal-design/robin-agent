...
-- Run this after creating the tables. The app.current_tenant setting is injected
-- by the withTenant() wrapper (SET LOCAL app.current_tenant = '<uuid>') before
-- every pooled query. Queries that don't set this GUC will see no rows
-- (fail-closed), which is the safe default.
--
-- IMPORTANT: the GUC name is app.current_tenant — NOT app.current_tenant.
-- withTenant.ts, phase4a/4b/4c migrations, and this file must all use the same name.
